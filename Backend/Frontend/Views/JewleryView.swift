...
struct JewleryView: View {
    
    @State private var jewlery = [ClothingItem]()
            ...
            if(userInfo!.id != nil) {
                ...
                ScrollView{
                    ...
                    do {
                        ...
                        let query = closet.whereField("clothing", isEqualTo: "jewlery")
                        jewlery = try await UserManager.shared.getClothingItems(userID: userInfo!.id!, query: query)
                    } catch {
                        jewlery = []
                        print("error loading data in short sleeve view")
                    }
                }.navigationTitle("My Jewlery")
            }
        } else {
