...
struct JumpSuitView: View {
    
    @State private var jumpsuit = [ClothingItem]()
            ...
            if(userInfo!.id != nil) {
                ...
                ScrollView{
                    ...
                    VStack(alignment: .leading, spacing: 20){
                        ...
                        // .scaledToFit()
                        ForEach(jumpsuit, id:\.id) { item in
                            if (item.img_url != nil){
                    ...
                    do {
                        ...
                        let query = closet.whereField("clothing", isEqualTo: "jumpsuit")
                        jumpsuit = try await UserManager.shared.getClothingItems(userID: userInfo!.id!, query: query)
                    } catch {
                        jumpsuit = []
                        print("error loading data in short sleeve view")
                    }
                }.navigationTitle("My Jumpsuits")
            }
        } else {
