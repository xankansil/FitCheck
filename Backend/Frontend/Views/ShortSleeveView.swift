...
struct ShortSleeveView: View {
    ...
    @State private var shortsleeves = [ClothingItem]()
            ...
            if(userInfo!.id != nil) {
                ...
                ScrollView{
                    ...
                    VStack(alignment: .leading, spacing: 20){
                        ...
                        // .scaledToFit()
                        ForEach(shortsleeves, id:\.id) { item in
                            if (item.img_url != nil){
                    ...
                    do {
                        ...
                        let query = closet.whereField("clothing", isEqualTo: "shortsleeve")
                        shortsleeves = try await UserManager.shared.getClothingItems(userID: userInfo!.id!, query: query)
                    } catch {
                        shortsleeves = []
                        print("error loading data in short sleeve view")
                    }
                }.navigationTitle("My Short Sleeves")
            }
        } else {
