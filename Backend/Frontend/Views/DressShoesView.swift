...
struct DressShoesView: View {
    
    @State private var dressshoes = [ClothingItem]()
            ...
            if(userInfo!.id != nil) {
                ...
                ScrollView{
                    ...
                    VStack(alignment: .leading, spacing: 20){
                        ...
                        // .scaledToFit()
                        ForEach(dressshoes, id:\.id) { item in
                            if (item.img_url != nil){
                    ...
                    do {
                        ...
                        let query = closet.whereField("clothing", isEqualTo: "dressshoes")
                        dressshoes = try await UserManager.shared.getClothingItems(userID: userInfo!.id!, query: query)
                    } catch {
                        dressshoes = []
                        print("error loading data in short sleeve view")
                    }
                }.navigationTitle("My Dress Shoes")
            }
        } else {
