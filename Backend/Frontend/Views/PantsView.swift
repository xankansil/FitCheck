...
struct PantsView: View {
    ...
    @State private var pants = [ClothingItem]()
            ...
            if(userInfo!.id != nil) {
                ...
                ScrollView{
                    ...
                    do {
                        ...
                        let query = closet.whereField("clothing", isEqualTo: "pants")
                        pants = try await UserManager.shared.getClothingItems(userID: userInfo!.id!, query: query)
                    } catch {
                        pants = []
                        print("error loading data in short sleeve view")
                    }
                }.navigationTitle("My Pants")
            }
        } else {
