...
struct ScarfsView: View {
    
    @State private var scarfs = [ClothingItem]()
            ...
            if(userInfo!.id != nil) {
                ...
                ScrollView{
                    ...
                    do {
                        ...
                        let query = closet.whereField("clothing", isEqualTo: "scarf")
                        scarfs = try await UserManager.shared.getClothingItems(userID: userInfo!.id!, query: query)
                    } catch {
                        scarfs = []
                        print("error loading data in short sleeve view")
                    }
                }.navigationTitle("My Scarfs")
            }
        } else {
