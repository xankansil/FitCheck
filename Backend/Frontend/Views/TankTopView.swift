...
struct TankTopView: View {
    
    @State private var tanktops = [ClothingItem]()
            ...
            if(userInfo!.id != nil) {
                ...
                ScrollView{
                    ...
                    VStack(alignment: .leading, spacing: 20){
                        ...
                        // .scaledToFit()
                        ForEach(tanktops, id:\.id) { item in
                            if (item.img_url != nil){
                    ...
                    do {
                        ...
                        let query = closet.whereField("clothing", isEqualTo: "sweater")
                        tanktops = try await UserManager.shared.getClothingItems(userID: userInfo!.id!, query: query)
                    } catch {
                        tanktops = []
                        print("error loading data in short sleeve view")
                    }
                }.navigationTitle("My Tank Tops")
            }
        } else {
